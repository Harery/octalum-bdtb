# Implementation Plan — SaaS for indie therapists
...
_Rationale: optimised for time-to-first-deploy on `saas` workloads.
Every entry is swappable — see `research.md` for the trade-offs._

## Architecture sketch

- **Entry point(s):** depends on domain — for `saas`, see the tree below.
...
A starter layout for `saas` projects:
